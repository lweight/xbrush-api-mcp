...
```
src/
├── index.ts              ← 서버 엔트리, 5개 도구 모듈 등록
├── constants.ts          ← API 베이스 URL, 제한값, 타임아웃
├── types.ts              ← 공통 타입 정의
├── schemas/              ← Zod 입력 스키마
│   ├── file-upload.ts
│   ├── image.ts
...
└── tools/                ← MCP 도구 핸들러
    ...
    ├── image.ts          ← generate, edit, upscale, remove_bg (각 sync/async 지원)
...
- **타임아웃**: sync 120초, video sync 600초, async POST 30초, GET 10초
- **입력 검증**: Zod strict mode (미정의 필드 거부)
- **Tool annotations**: `readOnlyHint`, `destructiveHint`, `idempotentHint` 명시

## Sync vs Async
- **기본 Sync**: `image_generate`, `image_remove_bg` — 결과 즉시 반환
- **기본 Async**: `image_edit`, `image_upscale`, `video_generate`, `video_upscale` — request ID 반환 후 `get_request`로 폴링
- `image_generate`, `image_edit`, `image_upscale`, `video_generate`, `video_upscale`은 `sync` 파라미터로 모드 전환 가능
...
- 커밋 메시지: 한국어
- Transport: stdio 전용
- 도구 11개 (Image 4, Video 2, Requests 3, Models 1, File Upload 1)
