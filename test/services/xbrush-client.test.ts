...
describe("handleApiError", () => {
  it("구조화된 에러 응답 — INVALID_API_KEY", () => {
    ...
    expect(result.suggestion).toContain("xbrush.run/api-keys");
  });
  ...
  it("구조화된 에러 응답 — INSUFFICIENT_CREDIT", () => {
    ...
    expect(result.suggestion).toContain("xbrush.run");
  });
...
describe("formatAsyncResult", () => {
  it("모든 메타 + get_request 안내 포함", () => {
    const resp: XBrushAsyncResponse = {
      requestId: "req" + "q".repeat(21),
      status: "pending",
      domain: "image",
      action: "edit",
      creditCharged: 5,
    ...
    const text = formatAsyncResult(resp, "Image edit");
    expect(text).toContain("Image edit submitted (async)");
    expect(text).toContain(resp.requestId);
